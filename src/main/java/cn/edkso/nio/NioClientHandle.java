...
public class NioClientHandle implements Runnable{
    ...
    private void handleInput(SelectionKey key) throws IOException {
        if(key.isValid()){
            ...
            //2. 处理连接就绪事件,但是三次握手未必就成功了，所以需要等待握手完成和判断握手是否成功
            if(key.isConnectable()){
                //2-1. 通过调用finishConnect方法完成连接操作
                //2-1. (finishConnect的主要作用就是确认通道连接已建立，
                //2-1. 方便后续IO操作（读写）不会因连接没建立而导致NotYetConnectedException异常。)
                if(sc.finishConnect()){
                    //2-1-1. 连接既然已经建立，当然就需要注册读事件，写事件一般是不需要注册的。
            ...
            //3. 处理读事件，也就是当前有数据可读
            if(key.isReadable()){
                //3-1. 创建ByteBuffer，并开辟一个1k的缓冲区
                ByteBuffer byteBuffer = ByteBuffer.allocate(1024);
                //3-2. 将通道的数据读取到缓冲区，read方法返回读取到的字节数
